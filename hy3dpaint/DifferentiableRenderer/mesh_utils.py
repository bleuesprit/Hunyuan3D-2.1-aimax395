...
# 尝试导入 bpy（Blender Python），如果不可用则使用 trimesh 作为 fallback
try:
    import bpy
    BPY_AVAILABLE = True
except ImportError:
...
def _setup_blender_scene():
    ...
    if not BPY_AVAILABLE:
        raise ImportError("bpy not available")
...
def _convert_obj_to_glb_bpy(
    ...
    """Convert OBJ to GLB using Blender bpy (primary method)."""
    try:
...
def _convert_obj_to_glb_trimesh(
    obj_path: str,
    glb_path: str,
    shade_type: str = "SMOOTH",
    auto_smooth_angle: float = 60,
    ...
    """Convert OBJ to GLB using trimesh + pymeshlab (fallback method)."""
    try:
        import trimesh
        import pymeshlab
        ...
        if shade_type == "SMOOTH":
            ...
            # 用 pymeshlab 做按角度自动平滑
            ms = pymeshlab.MeshSet()
            ms.add_mesh(pymeshlab.Mesh(mesh.vertices, mesh.faces))
            ms.compute_normal_by_angle(thresholddeg=auto_smooth_angle)
            m = ms.current_mesh()
        ...
        print(f"trimesh OBJ→GLB 转换失败: {e}")
        return False
...
def convert_obj_to_glb(
    ...
    优先使用 Blender bpy，如果不可用则 fallback 到 trimesh + pymeshlab。

    Args:
        obj_path: 输入 OBJ 文件路径
        glb_path: 输出 GLB 文件路径
        shade_type: 法线类型 ("SMOOTH", "FLAT", "AUTO_SMOOTH")
        auto_smooth_angle: 自动平滑角度（度）
        merge_vertices: 是否合并重复顶点
    ...
    if BPY_AVAILABLE:
        return _convert_obj_to_glb_bpy(
            obj_path, glb_path, shade_type, auto_smooth_angle, merge_vertices
        )
    else:
