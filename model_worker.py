...
class ModelWorker:
    ...
    def __init__(self,
        ...
        # Save texture generation config (lazy-load to save VRAM)
        self.model_path = model_path
    ...
    def _load_shape_pipeline(self):
        """Load (or reload) shape generation pipeline onto GPU."""
        if self.pipeline is not None:
            return
        self.pipeline = Hunyuan3DDiTFlowMatchingPipeline.from_pretrained(self.model_path)
        ...
        logger.info(f"Shape pipeline loaded. VRAM: {torch.cuda.memory_allocated() / 1024**3:.2f} GiB allocated")

    def _reload_shape_pipeline(self):
        """Alias for _load_shape_pipeline (called after paint)."""
        self._load_shape_pipeline()

    def _load_paint_pipeline(self):
        """Lazy-load texture generation pipeline (with reduced VRAM footprint)."""
        if self.paint_pipeline is not None:
            return
        max_num_view = 4
        resolution = 384
        conf = Hunyuan3DPaintConfig(max_num_view, resolution)
        conf.realesrgan_ckpt_path = os.path.join(self.model_path, "RealESRGAN_x4plus", "RealESRGAN_x4plus.pth")
        conf.multiview_cfg_path = "hy3dpaint/cfgs/hunyuan-paint-pbr.yaml"
        conf.custom_pipeline = "hy3dpaint/hunyuanpaintpbr"
        conf.multiview_pretrained_path = self.model_path
        conf.dino_ckpt_path = os.path.join(self.model_path, "dinov2-giant")
        # Reduce render/texture size to lower VRAM usage
        conf.render_size = 1024
        conf.texture_size = 1024
        self.paint_pipeline = Hunyuan3DPaintPipeline(conf)
        torch.cuda.empty_cache()
        logger.info(f"Paint pipeline loaded. VRAM: {torch.cuda.memory_allocated() / 1024**3:.2f} GiB allocated")

    def _unload_paint_pipeline(self):
        ...
        if self.paint_pipeline is not None:
            ...
            gc.collect()
            torch.cuda.empty_cache()
            logger.info(f"Paint pipeline unloaded. VRAM: {torch.cuda.memory_allocated() / 1024**3:.2f} GiB allocated")
    ...
    def generate(self, uid, params):
        ...
        # Unload shape model entirely and load paint pipeline (mutually exclusive for VRAM)
        del mesh
        shape_pipeline = self.pipeline
        self.pipeline = None
        del shape_pipeline
        gc.collect()
        torch.cuda.empty_cache()
        logger.info(f"VRAM after shape unload: {torch.cuda.memory_allocated() / 1024**3:.2f} GiB allocated")
        ...
        # Unload paint pipeline and reload shape pipeline for next generation
